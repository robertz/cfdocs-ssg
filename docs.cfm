...
<cfoutput>
		...
		<div class="container" data-doc="#encodeForHTMLAttribute(prc.name)#">
			...
			<cfif StructKeyExists(prc, "syntax") AND Len(prc.syntax)>
				...
				<cfif prc.type IS "tag">
					...
					<cfif StructKeyExists(prc, "script")>
						<p id="script-syntax">
							<small><span class="glyphicon glyphicon-flash" title="#encodeForHTMLAttribute(prc.scriptTitle)#"></span></small> &nbsp;<code>#encodeForHTML(prc.script)#</code>
						</p>
					</cfif>
				<cfelseif prc.type is "function">
					<cfif StructKeyExists(prc, "member")>
						...
						<p id="member-syntax">
							...
							<cfif StructKeyExists(prc, "member_details") AND StructKeyExists(prc.member_details, "returns") && Len(prc.member_details.returns)>
			...
			<cfif structKeyExists(prc, "engines") AND structKeyExists(prc.engines, "coldfusion") AND structKeyExists(prc.engines.coldfusion, "deprecated") AND len(prc.engines.coldfusion.deprecated)>
				<div class="alert alert-danger">
					<h4>
						<span class="glyphicon glyphicon-warning-sign"></span>
						<cfif structKeyExists(prc.engines.coldfusion, "removed") AND len(prc.engines.coldfusion.removed)>
							The #prc.name# <cfif prc.type IS "tag">tag<cfelseif prc.type IS "function">function</cfif> was <strong>REMOVED</strong> from ColdFusion #encodeForHTML(prc.engines.coldfusion.removed)# <cfif prc.engines.coldfusion.removed IS NOT prc.engines.coldfusion.deprecated> and has been <strong>DEPRECATED</strong> since ColdFusion #encodeForHTML(prc.engines.coldfusion.deprecated)#</cfif>
						<cfelse>
							The #prc.name# <cfif prc.type IS "tag">tag<cfelseif prc.type IS "function">function</cfif> is <strong>DEPRECATED</strong> as of ColdFusion #encodeForHTML(prc.engines.coldfusion.deprecated)#
						</cfif>
					</h4>
				</div>
			<cfelseif StructKeyExists(prc, "engines") AND structCount(prc.engines) EQ 1>
				<div class="alert alert-warning">
					...
					#engineMap[engine]#<cfif StructKeyExists(prc.engines[engine], "minimum_version") AND Len(prc.engines[engine].minimum_version)> #prc.engines[engine].minimum_version# and up</cfif>.&nbsp;
			...
			<cfif StructKeyExists(prc, "discouraged") AND Len(prc.discouraged)>
				<div class="alert alert-danger">
					<h4><span class="glyphicon glyphicon-warning-sign"></span> Discouraged: #autoLink(encodeForHTML(prc.discouraged))#</h4>
				</div>
			</cfif>
		</div>
	</div>
	...
	<cfif prc.type IS NOT "404" AND prc.type IS NOT "index">
		<ol class="breadcrumb container">
			...
			<cfif prc.type IS "function" OR prc.name contains "-functions"><li><a href="#linkTo("functions")#">Functions</a></li></cfif>
			<cfif prc.type IS "tag" OR prc.name contains "-tags"><li><a href="#linkTo("tags")#">Tags</a></li></cfif>
			<cfset cat = findCategory(prc.name)>
			<cfif Len(cat)><li><a href="#linkTo(cat)#">#collections.global.categories[cat].name#</a></li></cfif>
			<li class="active">#prc.name#</li>

			<cfif StructKeyExists(prc, "engines") AND StructKeyExists(prc.engines, "coldfusion") AND StructKeyExists(prc.engines.coldfusion, "docs") AND Len(prc.engines.coldfusion.docs)>
				<li class="pull-right">
					<a href="#prc.engines.coldfusion.docs#" title="Official Adobe ColdFusion Docs" class="label label-acf">CF<cfif StructKeyExists(prc.engines.coldfusion, "minimum_version") AND Len(prc.engines.coldfusion.minimum_version)>#encodeForHTML(prc.engines.coldfusion.minimum_version)#+</cfif></a>
				</li>
			</cfif>
			<cfif StructKeyExists(prc, "engines") AND StructKeyExists(prc.engines, "lucee") AND StructKeyExists(prc.engines.lucee, "docs") AND Len(prc.engines.lucee.docs)>
				<li class="pull-right">
					<a href="#prc.engines.lucee.docs#" title="Official Lucee Docs" class="label label-lucee">Lucee<cfif StructKeyExists(prc.engines.lucee, "minimum_version") AND Len(prc.engines.lucee.minimum_version)>#encodeForHTML(prc.engines.lucee.minimum_version)#+</cfif></a>
				</li>
			</cfif>
			<cfif StructKeyExists(prc, "engines") AND StructKeyExists(prc.engines, "openbd") AND StructKeyExists(prc.engines.openbd, "docs") AND Len(prc.engines.openbd.docs)>
				<li class="pull-right">
					<a href="#prc.engines.openbd.docs#" title="Official OpenBD Docs" class="label label-openbd">BD</a>
				</li>
			</cfif>
			...
			<li class="pull-right">
				<span class="label label-warning">
					<a href="https://github.com/foundeo/cfdocs/issues?q=is:issue%20is:open%20#encodeForURL(prc.name)#" class="issuecount badge">0</a>
					<a href="https://github.com/foundeo/cfdocs/issues/new?title=#encodeForURL(prc.name)#" rel="nofollow" class="issuebutton" title="Report an Issue">Issue</a>
				</span>
			</li>
			<cfif StructKeyExists(request,"gitFilePath") AND Len(request.gitFilePath) AND not (REFind("(cf|lucee)[0-9]{1,3}",prc.name) OR ArrayContains(['tags','functions','guides','all','categories'],prc.name) )>
				<li class="pull-right">
	...
	<div class="container">
		...
		<cfif StructKeyExists(prc, "engines")>
			...
			<cfloop index="i" list="#StructKeyList(prc.engines)#">
				<cfif (StructKeyExists(prc.engines[i], "notes") AND Len(prc.engines[i].notes)) OR ( StructKeyExists(prc.engines[i], "deprecated") AND Len(prc.engines[i].deprecated) )>
					<cfsavecontent variable="compatibilityData">
						...
						<div class="row">
							...
							<div class="col-sm-8">
								<cfif StructKeyExists(prc.engines[i], "deprecated") AND Len(prc.engines[i].deprecated)>
									<div class="alert alert-danger">
										<strong>DEPRECATED</strong> since version #encodeForHTML(prc.engines[i].deprecated)#
										<cfif StructKeyExists(prc.engines[i], "removed") AND Len(prc.engines[i].removed)>
											<strong>REMOVED</strong> in version #encodeForHTML(prc.engines[i].removed)#
										</cfif>
										<cfif StructKeyExists(prc.engines[i], "notes") AND Len(prc.engines[i].notes)>
											#autoLink(encodeForHTML(prc.engines[i].notes))#
										</cfif>
									</div>
								<cfelse>
									<div class="alert alert-warning">
										<cfif Len(prc.engines[i].minimum_version)><span class="label label-danger">Version #encodeForHTML(prc.engines[i].minimum_version)#+</span></cfif>
										#autoLink(encodeForHTML(prc.engines[i].notes))#
									</div>
								</cfif>
							</div>
		...
		<cfif StructKeyExists(request,'gitFilePath') and StructKeyExists(prc, "type") and (prc.type is "tag" or prc.type is "function")>
			<h2 id="examples">
				Examples
				...
				<div>
					<small>Sample code <cfif prc.type IS "function">invoking<cfelse>using</cfif> the #prc.name# <cfif prc.type IS "tag">tag<cfelse>function</cfif></small>
				</div>
			</h2>
						...
						<div class="row form">
							...
							<div class="col-md-5">
								<ol style="margin-left: -21px;">
									...
									<li><a href="https://github.com/foundeo/cfdocs#request.gitFilePath#" target="_blank" rel="nofollow" class="label label-danger">Click Here to edit doc for: #prc.name#</a></li>
						...
						<div class="panel-body">
							<cfif NOT structKeyExists(ex, "runnable") OR ex.runnable>
								<div class="pull-right">
									<button class="example-btn btn btn-default" data-name="#encodeForHTMLAttribute(LCase(prc.name))#" data-index="#example_index#"><span class="glyphicon glyphicon-play-circle"></span>&nbsp; Run Code</button>
								</div>
							</cfif>
